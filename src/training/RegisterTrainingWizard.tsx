...
export function RegisterTrainingWizard({
  ...
  const qualificationQuery = useQuery({
    queryKey: ["credentials-personnel", state.instructorStaffMemberId, "register-wizard"],
    queryFn: () => getPersonnelCredentials(state.instructorStaffMemberId),
    enabled: canViewCertifications && state.sessionMode === "NEW" && Boolean(state.instructorStaffMemberId),
    retry: false
  });
  ...
  const personnel = (personnelQuery.data?.personnel ?? []).filter((person) =>
    person.employment_status === "ACTIVE" && (!state.clientId || person.client_id === state.clientId)
  );
  const currentTime = Date.now();
  const qualifications = (qualificationQuery.data?.certifications ?? []).filter((certification) =>
    (certification.certification_level === "L6" || certification.certification_level === "L7") &&
    certification.certification_status === "ACTIVE" &&
    new Date(certification.issue_date).getTime() <= currentTime &&
    new Date(certification.expiry_date).getTime() > currentTime
  );
  ...
  function changeFacility(facilityId: string) { update({ facilityId, sessionId: "" }); }
  function changeProgram(programCode: TrainingProgramCode | "") { update({ programCode, sessionId: "" }); }
  ...
  function changeTrainer(instructorStaffMemberId: string) { update({ instructorStaffMemberId, qualificationCertificationId: "" }); }
  function next() {
  ...
  return <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-primary-navy/40 p-4 sm:items-center" role="presentation">
    <div aria-labelledby="register-training-title" aria-modal="true" className="w-full max-w-3xl rounded-component bg-white shadow-xl" onKeyDown={key} ref={dialogRef} role="dialog" tabIndex={-1}>
      <form onSubmit={complete}>
        <header className="border-b border-border p-5"><p className="text-xs font-semibold uppercase tracking-wide text-primary-blue">Training registration</p><h2 className="mt-1 text-xl font-semibold text-primary-navy" id="register-training-title">Register Training</h2>
          <ol aria-label="Registration progress" className="mt-4 flex flex-wrap gap-2 text-xs">{steps.map((item, position) => <li aria-current={item === step ? "step" : undefined} className={item === step ? "font-semibold text-primary-blue" : "text-text-muted"} key={item}>{position + 1}. {label(item)}</li>)}</ol>
        </header>
        <div className="max-h-[65vh] overflow-y-auto p-5"><p className="mb-4 text-sm font-semibold text-primary-navy">Current step: {label(step)}</p>
          ...
          {step === "PROGRAM" ? <ProgramStep state={state} clients={clients} facilities={facilities} changeClient={changeClient} changeFacility={changeFacility} changeProgram={changeProgram}/> : null}
          ...
          {step === "TRAINER" ? <TrainerStep state={state} personnel={personnel.filter((person) => Boolean(person.user_id))} qualifications={qualifications} loading={qualificationQuery.isLoading} changeTrainer={changeTrainer} update={update}/> : null}
          {step === "SCHEDULE" ? <ScheduleStep state={state} update={update}/> : null}
          {step === "REVIEW" ? <Review state={state} trainees={trainees} clients={clients} facilities={facilities} sessions={eligibleSessions} personnel={personnel} qualifications={qualifications}/> : null}
          {error ? <p className="mt-4 rounded-component border border-accent-red/30 bg-red-50 p-3 text-sm text-red-800" role="alert">{error} Retry uses the same frozen registration intent.</p> : null}
        </div>
        <footer className="flex flex-wrap justify-between gap-3 border-t border-border p-5"><Button disabled={pending || Boolean(intentRef.current)} onClick={cancel} type="button" variant="secondary">Cancel</Button><div className="flex gap-2">{index > 0 ? <Button disabled={pending || Boolean(intentRef.current)} onClick={back} type="button" variant="secondary">← Back</Button> : null}<Button disabled={pending || !validStep(step, state)} onClick={step === "REVIEW" ? undefined : next} type={step === "REVIEW" ? "submit" : "button"}>{pending ? "Completing…" : step === "REVIEW" ? "Complete Training Registration" : "Next →"}</Button></div></footer>
      </form>
    </div>
  </div>;
}
...
function buildSession(state: WizardState) { return { training_title: state.title.trim(), operational_skill: state.operationalSkill, training_start_date: state.startDate, training_end_date: state.endDate || null, duration_minutes: state.durationMinutes ? Number(state.durationMinutes) : null, facility_id: state.facilityId || null, instructor_staff_member_id: state.instructorStaffMemberId, instructor_qualification_certification_id: state.qualificationCertificationId, training_notes: state.sessionNotes.trim() || null }; }
...
function validStep(step: Step, state: WizardState) {
  ...
  if (step === "SESSION") return state.sessionMode === "EXISTING" ? Boolean(state.sessionId) : true;
  if (step === "TRAINER") return Boolean(state.instructorStaffMemberId && state.qualificationCertificationId);
  if (step === "SCHEDULE") return Boolean(state.title.trim() && state.startDate && state.operationalSkill);
  return true;
}
...
function TraineeStep({state,trainees,personnel,canLinkPersonnel,canRegisterTrainee,update}:{state:WizardState;trainees:readonly TrainingTrainee[];personnel:readonly RegistrationPersonnel[];canLinkPersonnel:boolean;canRegisterTrainee:boolean;update:(patch:Partial<WizardState>)=>void}) { return <div className="space-y-4"><fieldset><legend className="text-sm font-semibold">Trainee source</legend><div className="mt-2 flex gap-5"><label><input checked={state.traineeMode==="EXISTING"} name="trainee-mode" onChange={()=>update({traineeMode:"EXISTING",staffMemberId:""})} type="radio"/> Existing Trainee</label>{canRegisterTrainee?<label><input checked={state.traineeMode==="NEW"} name="trainee-mode" onChange={()=>update({traineeMode:"NEW",traineeId:""})} type="radio"/> Register New Trainee</label>:null}</div></fieldset>{state.traineeMode==="EXISTING"?<Field label="Existing Trainee"><select className={control} onChange={e=>update({traineeId:e.target.value})} value={state.traineeId}><option value="">Select a Trainee</option>{trainees.map(x=><option key={x.id} value={x.id}>{x.full_name} · {x.student_number ?? "Student number pending"}</option>)}</select></Field>:<div className="grid gap-4 sm:grid-cols-2"><Field label="Full name"><input className={control} onChange={e=>update({fullName:e.target.value})} value={state.fullName}/></Field><Field label="Email"><input className={control} onChange={e=>update({email:e.target.value})} type="email" value={state.email}/></Field><Field label="Phone number"><input className={control} onChange={e=>update({phoneNumber:e.target.value})} value={state.phoneNumber}/></Field><Field label="Notes"><input className={control} onChange={e=>update({notes:e.target.value})} value={state.notes}/></Field>{canLinkPersonnel?<Field label="Link Personnel (optional)"><select className={control} onChange={e=>update({staffMemberId:e.target.value})} value={state.staffMemberId}><option value="">No Personnel link</option>{personnel.map(x=><option key={x.id} value={x.id}>{x.full_name}</option>)}</select></Field>:null}</div>}</div>; }
function ProgramStep({state,clients,facilities,changeClient,changeFacility,changeProgram}:{state:WizardState;clients:readonly RegistrationClient[];facilities:readonly RegistrationFacility[];changeClient:(id:string)=>void;changeFacility:(id:string)=>void;changeProgram:(code:TrainingProgramCode|"")=>void}) { return <div className="grid gap-4 sm:grid-cols-2"><Field label="Program"><select className={control} onChange={e=>changeProgram(e.target.value as TrainingProgramCode|"")} value={state.programCode}><option value="">Select a Program</option>{trainingProgramOptions.map(x=><option key={x.program_code} value={x.program_code}>{x.certification_level} · {x.display_name}</option>)}</select></Field><Field label="Sponsoring Client (optional)"><select className={control} onChange={e=>changeClient(e.target.value)} value={state.clientId}><option value="">No sponsoring Client</option>{clients.map(x=><option key={x.id} value={x.id}>{x.organization_name}</option>)}</select></Field><Field label="Facility (optional)"><select className={control} disabled={!state.clientId} onChange={e=>changeFacility(e.target.value)} value={state.facilityId}><option value="">No Facility</option>{facilities.map(x=><option key={x.id} value={x.id}>{x.facility_name}</option>)}</select></Field></div>; }
function SessionStep({state,sessions,canCreateSession,changeMode,update}:{state:WizardState;sessions:readonly TrainingSession[];canCreateSession:boolean;changeMode:(mode:SessionMode)=>void;update:(patch:Partial<WizardState>)=>void}) { return <div className="space-y-4"><fieldset><legend className="text-sm font-semibold">Training Session source</legend><div className="mt-2 flex gap-5"><label><input checked={state.sessionMode==="EXISTING"} name="session-mode" onChange={()=>changeMode("EXISTING")} type="radio"/> Use Existing Training Session</label>{canCreateSession?<label><input checked={state.sessionMode==="NEW"} name="session-mode" onChange={()=>changeMode("NEW")} type="radio"/> Create New Training Session</label>:null}</div></fieldset>{state.sessionMode==="EXISTING"?<><Field label="Eligible Training Session"><select className={control} onChange={e=>update({sessionId:e.target.value})} value={state.sessionId}><option value="">Select a Session</option>{sessions.map(x=><option key={x.id} value={x.id}>{x.training_title}</option>)}</select></Field>{sessions.length===0?<div className="rounded-component border border-dashed border-border p-4"><p>No eligible Training Session is available for this selection.</p>{canCreateSession?<Button className="mt-3" onClick={()=>changeMode("NEW")} type="button" variant="secondary">Create New Training Session</Button>:null}</div>:null}</>:<p className="text-sm text-text-muted">The new Session will be created only when you complete the reviewed registration.</p>}</div>; }
function TrainerStep({state,personnel,qualifications,loading,changeTrainer,update}:{state:WizardState;personnel:readonly RegistrationPersonnel[];qualifications:readonly {id:string;business_identifier:string;certification_level:string}[];loading:boolean;changeTrainer:(id:string)=>void;update:(patch:Partial<WizardState>)=>void}) { return <div className="space-y-4"><Field label="Primary Instructor"><select className={control} onChange={e=>changeTrainer(e.target.value)} value={state.instructorStaffMemberId}><option value="">Select active linked Personnel</option>{personnel.map(x=><option key={x.id} value={x.id}>{x.full_name}</option>)}</select></Field><Field label="Exact qualifying Certification"><select className={control} disabled={!state.instructorStaffMemberId||loading} onChange={e=>update({qualificationCertificationId:e.target.value})} value={state.qualificationCertificationId}><option value="">Select current L6/L7 Certification</option>{qualifications.map(x=><option key={x.id} value={x.id}>{x.business_identifier} · {x.certification_level}</option>)}</select></Field>{state.instructorStaffMemberId&&!loading&&qualifications.length===0?<p role="alert">This Personnel record has no eligible current L6/L7 Trainer Certification.</p>:null}</div>; }
function ScheduleStep({state,update}:{state:WizardState;update:(patch:Partial<WizardState>)=>void}) { return <div className="grid gap-4 sm:grid-cols-2"><Field label="Session title"><input className={control} onChange={e=>update({title:e.target.value})} value={state.title}/></Field><Field label="Operational skill"><select className={control} onChange={e=>update({operationalSkill:e.target.value as TrainingOperationalSkill})} value={state.operationalSkill}>{trainingOperationalSkills.map(x=><option key={x} value={x}>{x.replaceAll("_"," ")}</option>)}</select></Field><Field label="Start date"><input className={control} onChange={e=>update({startDate:e.target.value})} type="datetime-local" value={state.startDate}/></Field><Field label="End date"><input className={control} onChange={e=>update({endDate:e.target.value})} type="datetime-local" value={state.endDate}/></Field><Field label="Duration minutes"><input className={control} min="1" onChange={e=>update({durationMinutes:e.target.value})} type="number" value={state.durationMinutes}/></Field><Field label="Session notes"><input className={control} onChange={e=>update({sessionNotes:e.target.value})} value={state.sessionNotes}/></Field></div>; }
function Review({state,trainees,clients,facilities,sessions,personnel,qualifications}:{state:WizardState;trainees:readonly TrainingTrainee[];clients:readonly RegistrationClient[];facilities:readonly RegistrationFacility[];sessions:readonly TrainingSession[];personnel:readonly RegistrationPersonnel[];qualifications:readonly {id:string;business_identifier:string;certification_level:string}[]}) { const trainee=trainees.find(x=>x.id===state.traineeId),program=trainingProgramOptions.find(x=>x.program_code===state.programCode),session=sessions.find(x=>x.id===state.sessionId),trainer=personnel.find(x=>x.id===state.instructorStaffMemberId),qualification=qualifications.find(x=>x.id===state.qualificationCertificationId);return <dl className="grid gap-3 rounded-component bg-elevated p-4 text-sm sm:grid-cols-2"><Summary label="Trainee" value={state.traineeMode==="EXISTING"?`${trainee?.full_name} · ${trainee?.student_number??"No Student Number"}`:`${state.fullName} · new Student Number will be allocated`}/><Summary label="Program" value={program?`${program.certification_level} · ${program.display_name}`:""}/><Summary label="Sponsorship" value={`${clients.find(x=>x.id===state.clientId)?.organization_name??"None"} · ${facilities.find(x=>x.id===state.facilityId)?.facility_name??"No Facility"}`}/><Summary label="Training Session" value={state.sessionMode==="EXISTING"?`${session?.training_title} · existing record reused`:`${state.title} · new record`}/><Summary label="Primary Instructor" value={state.sessionMode==="EXISTING"?(session?.instructor_staff_member?.full_name??""):(trainer?.full_name??"")}/><Summary label="Qualification" value={state.sessionMode==="EXISTING"?(session?.instructor_qualification_certification?.business_identifier??""):(qualification?`${qualification.business_identifier} · ${qualification.certification_level}`:"")}/><Summary label="Enrollment" value="A new governed Enrollment will be created and assigned to this Session."/></dl>; }
